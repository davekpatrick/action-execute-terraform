...
// ------------------------------------
// ------------------------------------
module.exports = async function runProduct(
  argPathToBinary,
  argRunDirectory,
  argRunArguments
) {
  ...
  actionsExecOptions.cwd =
    process.env.GITHUB_WORKSPACE + path.sep + argRunDirectory
  // Execute and capture output
  actionsCore.debug('processEnv[' + JSON.stringify(process.env) + ']')
  actionsCore.debug(
    'actionsExecOptions[' + JSON.stringify(actionsExecOptions) + ']'
  )
  let actionExecExitCode = await actionsExec.exec(
    argPathToBinary,
    argRunArguments,
    actionsExecOptions
  )
