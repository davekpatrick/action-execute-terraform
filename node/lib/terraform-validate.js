...
// ------------------------------------
// ------------------------------------
module.exports = async function terraformValidate(
  argPathToBinary,
  argRunDirectory
) {
  ...
  let runProductInitData = await runProduct(argPathToBinary, argRunDirectory, [
    'init',
    '-backend=false',
  ])
  if (runProductInitData === undefined) {
    return
  }
  actionsCore.debug(
    'runProductInitData[' + JSON.stringify(runProductInitData) + ']'
  )
  ...
  var runProductData = await runProduct(argPathToBinary, argRunDirectory, [
    'validate',
    '-json',
  ])
  ...
  if (runProductData.exitCode !== 0) {
    ...
    if (runProductDataValid === false) {
      ...
      actionsCore.info(
        'Total Errors[' + JSON.parse(runProductData.stdOut).error_count + ']'
      )
      actionsCore.info(
        'Total Warnings[' +
          JSON.parse(runProductData.stdOut).warning_count +
          ']'
      )
    }
  } else {
