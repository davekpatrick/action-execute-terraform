...
module.exports = async function setupProduct(
  argProductName,
  argSetupVersion,
  ...
  actionsCore.debug(
    'osPlatform[' + osPlatform + '] osArchitecture[' + osArchitecture + ']'
  )
  if (argSetupVersion !== 'latest') {
    var setupVersionRange = semver.validRange(argSetupVersion, {
      argIncludePrerelease,
      loose: true,
    })
    var setupVersionValid = null
    var setupVersion = null
    if (setupVersionRange == null) {
      setupVersionValid = false
      if (argVersionInvalidHandling === 'fail') {
        ...
        actionsCore.warning(
          'Invalid version [' + argSetupVersion + '] using latest'
        )
        setupVersion = 'latest'
      } else {
        actionsCore.setFailed(
          'Invalid version[' +
            argSetupVersion +
            ']  unknown versionInvalidHandling[' +
            argVersionInvalidHandling +
            ']'
        )
        return
      ...
      setupVersionValid = true
      setupVersion = argSetupVersion
    }
  } else {
    setupVersionValid = false
    setupVersion = argSetupVersion
  ...
  let releaseData = await hashicorpReleases.getRelease(
    argProductName,
    setupVersion,
    argUserAgent,
    argIncludePrerelease
  ...
  if (!releaseData) {
    actionsCore.setFailed(
      'Unable to locate release data for ' + argProductName + ' ' + setupVersion
    )
    return
  }
  ...
  let cachedProductPath = actionsToolCache.find(
    argProductName,
    productVersion,
    osArchitecture
  )
  if (cachedProductPath) {
    ...
    // Extract the build
    var setupExtractSourceFilePath = null
    if (osPlatform === 'windows') {
      setupExtractSourceFilePath = downloadFilePath + '.zip'
        ...
        if (error) {
          actionsCore.setFailed(
            'Unable to rename file [' +
              downloadFilePath +
              '] to [' +
              setupExtractSourceFilePath +
              ']'
          )
          return
        }
        actionsCore.debug(
          'Successfully renamed file[' + setupExtractSourceFilePath + ']'
        )
      })
    } else {
      setupExtractSourceFilePath = downloadFilePath
    }
    actionsCore.debug(
      'setupExtractSourceFilePath[' + setupExtractSourceFilePath + ']'
    )
    var extractPath = await actionsToolCache.extractZip(
      setupExtractSourceFilePath,
      undefined
    )
    ...
    productPath = await actionsToolCache.cacheDir(
      extractPath,
      argProductName,
      productVersion,
      osArchitecture
  ...
  var productFilePath = null
  if (osPlatform === 'windows') {
    productFilePath = productPath + path.sep + argProductName + '.exe'
  } else {
    productFilePath = productPath + path.sep + argProductName
  }
